...
<div class="content-wrapper">
        ...
        <div class="card table-card">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h4 class="card-title">Faq Sub Course</h4>
                ...
                <div class="table-responsive">
                    <table id="programTable" class="table table-bordered table-hover align-middle">
                        ...
                        <tbody>
                            @foreach($faqSubCourse as $index => $faqSubCours)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ $faqSubCours->subCource ? $faqSubCours->subCource->sub_cource_name : 'N/A' }}</td>
                                    <td>{{ $faqSubCours->question }}</td>
                                    <td>{{ $faqSubCours->answer }}</td>
                                    <td>
                                        <span class="badge {{ $faqSubCours->status == 0 ? 'bg-success' : 'bg-secondary' }}">
                                            {{ $faqSubCours->status == 0 ? 'Active' : 'Inactive' }}
                                        </span>
                                    </td>
                                    ...
                                    <td>
                                        <button 
                                            ...
                                            data-sub_cource__id="{{ $faqSubCours->sub_cource__id }}"
                                            data-id="{{ $faqSubCours->id }}"
                                            data-answer="{{ $faqSubCours->answer }}"
                                            data-question="{{ $faqSubCours->question }}"
                                            data-status="{{ $faqSubCours->status }}"
    ...
    <div class="modal fade" id="addModal" tabindex="-1" aria-hidden="true">
      <div class="modal-dialog">
          <form action="{{ route('faq-subCourse.store') }}" method="POST" enctype="multipart/form-data" class="modal-content">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title">Add Faq Sub Course</h5>
                            ...
                            <div class="form-group">
                                ...
                                <select name="sub_cource__id" id="sub_cource__id" class="form-select">
                                    ...
                                    @if (!empty($subCources) && $subCources->count())
                                        @foreach ($subCources as $subCource)
                                            @if ($subCource)
                                                <option value="{{ $subCource->id }}">{{ $subCource->sub_cource_name }}</option>
                                            @endif
                                        @endforeach
                                    @endif
                                </select>
...
<script>
    $(document).ready(function () {
        $('.editBtn').on('click', function () {
            ...
            const subCourceId = $(this).data('sub_cource__id');
            const question = $(this).data('question');
            const answer = $(this).data('answer');
            const status = $(this).data('status');

            $('#edit_id').val(id);
            $('#edit_sub_cource__id').val(subCourceId);
            $('#edit_question').val(question);
            $('#edit_answer').val(answer);
            $('#edit_status').val(status);

            // Set the action URL dynamically
            $('#editForm').attr('action', `/faq-subCourse/${id}`);
        });
        ...
        $('#deleteModal').on('show.bs.modal', function (event) {
            ...
            var action = "{{ url('faq-subCourse') }}/" + id;
