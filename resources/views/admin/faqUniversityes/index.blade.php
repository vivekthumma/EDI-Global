...
<div class="content-wrapper">
        ...
        <div class="card table-card">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h4 class="card-title">FaqUniversity</h4>
                ...
                <div class="table-responsive">
                    <table id="programTable" class="table table-bordered table-hover align-middle">
                        ...
                        <tbody>
                            @foreach($faqUniversityes as $index => $faqUniversity)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ $faqUniversity->university ? $faqUniversity->university->name : 'N/A' }}</td>
                                    <td>{{ $faqUniversity->question }}</td>
                                    <td>{{ $faqUniversity->answer }}</td>
                                    <td>
                                        <span class="badge {{ $faqUniversity->status == 0 ? 'bg-success' : 'bg-secondary' }}">
                                            {{ $faqUniversity->status == 0 ? 'Active' : 'Inactive' }}
                                        </span>
                                    </td>
                                    ...
                                    <td>
                                        <button 
                                            ...
                                            data-university_id="{{ $faqUniversity->university_id }}"
                                            data-id="{{ $faqUniversity->id }}"
                                            data-answer="{{ $faqUniversity->answer }}"
                                            data-question="{{ $faqUniversity->question }}"
                                            data-status="{{ $faqUniversity->status }}"
    ...
    <div class="modal fade" id="addModal" tabindex="-1" aria-hidden="true">
      <div class="modal-dialog">
          <form action="{{ route('faq-universityes.store') }}" method="POST" enctype="multipart/form-data" class="modal-content">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title">Add Faq University</h5>
...
<script>
    $(document).ready(function () {
        $('.editBtn').on('click', function () {
            ...
            const universityId = $(this).data('university_id');
            ...
            $('#edit_id').val(id);
            $('#edit_university_id').val(universityId);
            $('#edit_question').val(question);
            $('#edit_answer').val(answer);
            $('#edit_status').val(status);

            // Dynamically set form action
            $('#editForm').attr('action', `/faq-universityes/${id}`);
        });
        ...
        $('#deleteModal').on('show.bs.modal', function (event) {
            ...
            var action = "{{ url('faq-universityes') }}/" + id;
